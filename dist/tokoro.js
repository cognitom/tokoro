...
normalize = function(str) {
  return new Moji(str).convert('HK', 'ZK').convert('ZE', 'HE').convert('ZS', 'HS').toString().replace(/\s+/g, '').replace(/(\d+)丁目(\d+)番(\d+)号/, function(_, m1, m2, m3) {
    return (china(m1)) + "丁目" + m2 + "-" + m3;
  }).replace(/(\d+)丁目(\d+)番/, function(_, m1, m2) {
    return (china(m1)) + "丁目" + m2;
  }).replace(/(\d+)丁目/, function(_, m1) {
    return (china(m1)) + "丁目";
